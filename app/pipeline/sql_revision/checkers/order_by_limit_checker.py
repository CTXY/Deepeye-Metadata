...
class OrderByLimitChecker(BaseChecker):
    ...
    def check_and_revise(self, sql: str, data_item: DataItem, llm: LLM, sampling_budget: int = 1) -> Tuple[str, Dict[str, int]]:
        ...
        if order_by_limit_suggestion:
            ...
            # Get SQL guidance (low confidence reference material, especially for ORDER BY LIMIT patterns)
            sql_guidance = PromptFactory.get_sql_guidance(data_item)
            prompt = PromptFactory.format_common_checker_prompt(database_schema_profile, data_item.question, data_item.evidence, sql, order_by_limit_suggestion, sql_guidance)
