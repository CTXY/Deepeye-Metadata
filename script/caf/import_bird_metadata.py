#!/usr/bin/env python3
"""
BIRD Dataset Metadata Import Script

This script imports metadata from BIRD dataset into SemanticMemoryStore.
It uses CAFSystem.import_bird_metadata() to process the database_description 
CSV files for each database and convert them to our metadata format.

IMPORTANT: This script only imports data that exists in the original BIRD files.
No additional information is constructed or generated. All fields not present
in the original CSV files are set to None/empty to allow for subsequent
generation and validation.

Usage:
    python script/caf/import_bird_metadata.py [--database-path PATH] [--database DATABASE_ID] [--data-dir DATA_DIR]

Examples:
    # Import from database file path
    python script/caf/import_bird_metadata.py --database-path /path/to/california_schools.sqlite
    
    # Import specific database by ID
    python script/caf/import_bird_metadata.py --database financial
    
    # Import all databases
    python script/caf/import_bird_metadata.py
    
    # Import with custom config
    python script/caf/import_bird_metadata.py --database california_schools --config /path/to/caf_config.yaml
    
    # Import from custom data directory
    python script/caf/import_bird_metadata.py --data-dir /path/to/bird/data
"""
...
class BirdMetadataImporter:
    """
    Imports BIRD dataset metadata into SemanticMemoryStore.
    
    This importer only uses data that exists in the original BIRD CSV files.
    ...
    def __init__(self, bird_data_dir: Path, config: CAFConfig, only_value_desc: Optional[List[str]] = None):
        self.bird_data_dir = bird_data_dir
        self.databases_dir = bird_data_dir / "databases" / "dev_databases"
        self.semantic_store = SemanticMemoryStore(config.memory)
        self.schema_extractor = DatabaseSchemaExtractor()
        # Optional filter: if provided, only extract value_description for matching columns
        # Accept fully-qualified names like "table.column" (case-insensitive) or wildcard per table: "table.*"
        ...
        # Initialize LLM analyzer from global LLM config for parsing value_description
        ...
        logger.info(f"BirdMetadataImporter initialized with data dir: {bird_data_dir}")
    ...
    def import_database_from_path(self, database_path: str) -> bool:
        """Import metadata for a database from its file path"""
        logger.info(f"Starting import for database path: {database_path}")
        ...
        try:
            ...
            self._import_with_schema_matching(actual_database_id, actual_schema, bird_database_id, database_path)
            
            logger.info(f"Successfully imported metadata for database: {actual_database_id}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to import database from {database_path}: {e}")
            return False
    ...
    def import_database(self, database_id: str) -> bool:
        """Import metadata for a specific database (legacy method for backward compatibility)"""
        logger.info(f"Starting import for database: {database_id}")
        
        db_dir = self.databases_dir / database_id
        desc_dir = db_dir / "database_description"
        
        if not desc_dir.exists():
            logger.error(f"Database description directory not found: {desc_dir}")
            return False
        ...
        try:
            ...
            if not db_files:
                logger.error(f"No database file found in {db_dir}")
                return False
            
            database_path = str(db_files[0])
            return self.import_database_from_path(database_path)
            
        except Exception as e:
            logger.error(f"Failed to import database {database_id}: {e}")
            return False
    
    def _import_with_schema_matching(self, actual_database_id: str, actual_schema: Dict, bird_database_id: str, database_path: str):
    ...
    def _import_table_metadata_with_mapping(self, actual_database_id: str, actual_table: str, 
                                          csv_file: Path, column_mapping: Dict[str, str],
        ...
        for row in rows:
                ...
                if conn is not None:
                    sample_values = self._get_column_sample_values(conn, actual_table, actual_column_name, limit=25)
                    ...
                    try:
                        ...
                        if enriched:
                            ...
                            if enriched_fields:
                                ...
                                self.semantic_store.add_column_metadata(enriched_metadata, source='llm_bird_value_desc')

                            # Handle any term definitions extracted from value_description
                            term_defs = enriched.get('term_definitions') or []
                            for td in term_defs:
                                try:
                                    term_name = (td.get('term_name') or '').strip()
                                    definition = (td.get('definition') or '').strip()
                                    formula = (td.get('formula') or None) or None
                                    example_usage = (td.get('example_usage') or None) or None
                                    ...
                                    context = f"BIRD value_description: table={actual_table}, column={actual_column_name}"
                                    ...
                                    logger.debug(f"Failed to add term definition from value_description for {actual_table}.{actual_column_name}: {e}")
                    except Exception as e:
    ...
    def _add_database_metadata(self, database_id: str) -> None:
        ...
        # Create database metadata - only use information from original files
    ...
    def _add_table_metadata(self, database_id: str, table_name: str) -> None:
        """Add table-level metadata"""
        # Only use information from original BIRD files - no table descriptions available
        table_metadata = TableMetadata(
            database_id=database_id,
            table_name=table_name,
            description=None  # No table descriptions in BIRD files
        )
        
        self.semantic_store.add_table_metadata(table_metadata, source='manual')
        logger.debug(f"Added table metadata for: {table_name}")
    
    def _add_columns_from_csv(self, database_id: str, table_name: str, csv_file: Path) -> int:
        """Add column metadata from BIRD CSV file"""
        columns_added = 0
        ...
        try:
            ...
            for row in rows:
                    # Extract BIRD fields exactly as they appear in CSV
                    original_column_name = row['original_column_name'].strip()
                    ...
                    # Prefer original CSV description initially
                    final_description = column_description if column_description else None

                    # Save the basic fields first (manual/original)
                    base_metadata = ColumnMetadata(
                        database_id=database_id,
                        table_name=table_name,
                        column_name=original_column_name,
                        whole_column_name=column_name,
                        description=final_description,
                        data_format=data_format if data_format else None
                    )
                    self.semantic_store.add_column_metadata(base_metadata, source='manual')

                    # If value_description contains richer semantics, use LLM to extract
                    if value_description:
                        try:
                            ...
                            # Build a minimal update only with non-empty enriched fields
                            ...
                            for td in term_defs:
                                try:
                                    ...
                                    context = f"BIRD value_description: table={table_name}, column={original_column_name}"
                                    ...
                                    logger.debug(f"Failed to add term definition from value_description for {table_name}.{original_column_name}: {e}")
                        except Exception as e:
                            logger.warning(f"LLM enrichment failed for {table_name}.{original_column_name}: {e}")
                    columns_added += 1
                    
                    logger.debug(f"Added column metadata: {table_name}.{original_column_name}")
    ...
    def _get_column_sample_values(self, conn: sqlite3.Connection, table_name: str, column_name: str, limit: int = 25) -> List[str]:
    ...
    def _filter_encoding_mapping(self, mapping: Dict[Any, Any], value_description: str, sample_values: Optional[List[str]]) -> Dict[str, str]:
    ...
    def _extract_from_value_description(self, table_name: str, column_name: str, value_description: str, column_description: str = "", sample_values: Optional[List[str]] = None) -> dict:
        ...
        - encoding_mapping
        - semantic_tags
        - term_definitions: list of business term definitions (for derived metrics, rates, business concepts)
        """
...
"""
        ...
        response_text = self.llm_analyzer._call_llm(prompt)  # reuse analyzer client
        # print('-------------Prompt for extracting from value description------------')
        # print(prompt)
        ...
        try:
            ...
            if isinstance(mapping, dict) and mapping:
                ...
                filtered_mapping = self._filter_encoding_mapping(mapping, value_description, sample_values)
                if filtered_mapping:
            ...
            # New: pass through term_definitions from LLM (no normalization beyond basic type check)
            ...
            # Check if we have any meaningful extracted fields
            ...
            if not has_meaningful_fields and value_description and value_description.strip():
                ...
                logger.debug(f"All LLM fields empty for {table_name}.{column_name}, storing value_description as semantic_tag")
            
            return result
        except Exception as e:
            logger.debug(f"Failed to parse LLM response for {table_name}.{column_name}: {e}; response={response_text[:200]}")
            return {}
...
"""
        ...
        print('-------------Prompt for extracting terms from evidence------------')
        print(prompt)
        ...
        try:
            ...
            logger.debug(f"Failed to parse LLM response for evidence terms: {e}; response={response_text[:200]}")
            return []
                ...
                for td in term_defs:
                    try:
                        ...
                        logger.warning(
                            ...
                            f"(db={bird_database_id}, qid={question_id}, term={td.get('term_name', 'unknown')}): {e}",
                            exc_info=True
                        )
            except Exception as e:
                logger.debug(f"Failed processing dev.json item: {e}")

        if imported_terms:
            logger.info(f"Imported {imported_terms} term definitions from dev.json for database {actual_database_id}")
    
    def import_all_databases(self) -> None:
        """Import metadata for all available databases"""
        available_databases = self.get_available_databases()
        logger.info(f"Found {len(available_databases)} databases to import")
        
        success_count = 0
        failed_databases = []
        
        for database_id in available_databases:
            if self.import_database(database_id):
                success_count += 1
            else:
                failed_databases.append(database_id)
        
        logger.info(f"Import completed: {success_count}/{len(available_databases)} databases imported successfully")
        
        if failed_databases:
            logger.warning(f"Failed to import: {', '.join(failed_databases)}")
    
    def get_import_summary(self) -> dict:
        """Get summary of what would be imported"""
        available_databases = self.get_available_databases()
        summary = {
...
def _find_database_file(bird_data_dir: Path, database_id: str) -> Optional[Path]:
    """Find database file for a given BIRD database ID"""
    db_dir = bird_data_dir / "databases" / "dev_databases" / database_id
...
def _get_available_databases(bird_data_dir: Path) -> List[str]:
    """Get list of available database IDs in BIRD dataset"""
    databases = []
    databases_dir = bird_data_dir / "databases" / "dev_databases"
...
def _get_import_summary(bird_data_dir: Path) -> dict:
    """Get summary of what would be imported"""
    databases_dir = bird_data_dir / "databases" / "dev_databases"
    available_databases = _get_available_databases(bird_data_dir)
    ...
    for database_id in available_databases:
        db_dir = databases_dir / database_id
        desc_dir = db_dir / "database_description"
...
def main():
    parser = argparse.ArgumentParser(description="Import BIRD dataset metadata into SemanticMemoryStore")
    parser.add_argument(
        '--database-path',
        type=str,
        help="Path to database file to import metadata for"
    )
    parser.add_argument(
        '--database', 
        type=str, 
        help="Specific database ID to import (if not provided, imports all)"
    )
    parser.add_argument(
        '--data-dir',
        type=Path,
        default=Path(__file__).parent.parent.parent / "dataset" / "bird",
        help="Path to BIRD dataset directory (default: ./dataset/bird)"
    )
    parser.add_argument(
        '--config',
        type=Path,
    ...
    parser.add_argument(
        '--summary',
        action='store_true',
        help="Show import summary without actually importing"
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
    ...
    parser.add_argument(
        '--force-regenerate',
        action='store_true',
        help="Force reimport even if metadata exists"
    )
    parser.add_argument(
    ...
    if not args.data_dir.exists():
        logger.error(f"BIRD data directory not found: {args.data_dir}")
        sys.exit(1)
    ...
    try:
        ...
        # Create CAFSystem instance (global config is automatically initialized internally)
        caf_system = CAFSystem(config)
        
        if args.summary:
            # Show summary
            summary = _get_import_summary(args.data_dir)
            print(f"\nImport Summary:")
            print(f"Total databases: {summary['total_databases']}")
            print(f"\nDatabases:")
            for db_id, info in summary['databases'].items():
                print(f"  {db_id}: {info['tables']} tables")
            sys.exit(0)
        ...
        if args.database_path:
            ...
            # Import specific database (legacy mode)
            available_databases = _get_available_databases(args.data_dir)
            if args.database not in available_databases:
                logger.error(f"Database '{args.database}' not found in BIRD dataset")
                sys.exit(1)
            
            db_file = _find_database_file(args.data_dir, args.database)
            if not db_file:
                logger.error(f"Database file not found for database '{args.database}'")
                sys.exit(1)
            database_paths = [str(db_file)]
        else:
            # Import all databases
            available_databases = _get_available_databases(args.data_dir)
            logger.info(f"Found {len(available_databases)} databases to import")
            ...
            for db_id in available_databases:
                db_file = _find_database_file(args.data_dir, db_id)
                if db_file:
                    database_paths.append(str(db_file))
                else:
                    logger.warning(f"Database file not found for database '{db_id}', skipping")
        
        # Perform import for each database
        success_count = 0
        failed_count = 0
        
        for db_path in database_paths:
            logger.info(f"\n{'='*80}")
            logger.info(f"Importing metadata for: {db_path}")
            logger.info(f"{'='*80}")
            
            result = caf_system.import_bird_metadata(
                database_path=db_path,
                bird_data_dir=str(args.data_dir),
            ...
            if result.get('success', False):
                success_count += 1
                logger.info(f"✅ Successfully imported metadata for: {db_path}")
            else:
                failed_count += 1
                logger.error(f"❌ Failed to import metadata for: {db_path}")
        ...
        logger.info(f"{'='*80}")
        logger.info(f"Total databases: {len(database_paths)}")
