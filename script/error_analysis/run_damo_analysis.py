"""
Run error analysis on DAMO format results

This script processes gpt_4o_mini_damo_wo_memory_results_on_train_set.json
"""
...
from script.error_analysis.damo_data_loader import DAMODataLoader
...
from script.error_analysis.models import Insight
from script.error_analysis import utils

logger = logging.getLogger(__name__)


class DAMOErrorAnalysisPipeline:
    """
    Error analysis pipeline for DAMO format data
    
    Uses the same processing components but different data loader
    """
    ...
    def __init__(self, args):
        ...
        self.data_loader = DAMODataLoader(
            results_path=Path(args.damo_results_path),
            database_mapping_path=Path(args.database_mapping) if args.database_mapping else None
        )
        
        # Initialize processing components (same as main.py)
    ...
    def run(self):
        ...
        logger.info("="*80)
        logger.info("DAMO Error Analysis Pipeline")
        logger.info("="*80)
        ...
        # Stage 2-5: Process each sample (same as main.py)
        processed_samples = []
    ...
    def _process_sample(self, sample):
        """Process a single sample through all stages (same logic as main.py)"""
        ...
        logger.info(f"  Intent: {miner_output.guidance.intent}")
    ...
    def _save_intermediate_sample(self, sample):
        """Save intermediate result for a single sample"""
        output_file = Config.INTERMEDIATE_DIR / f"damo_sample_{sample.question_id}.json"
        utils.save_json(sample, output_file)
    
    def _generate_and_save_insights(self, processed_samples):
        ...
        # Create one insight per sample
        insights = []
        
        for sample in successful_samples:
            insight = Insight(
                insight_id=f"damo_insight_{sample.question_id}",
    ...
    def _print_summary(self):
        ...
        logger.info(f"Non-generalizable filtered: {self.stats['non_generalizable_filtered']}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="DAMO Error Analysis Pipeline"
    )
    ...
    parser.add_argument(
        '--damo-results-path',
        type=str,
        default="/home/yangchenyu/Text2SQL/reasoning_modules/damo/results/gpt_4o_mini_damo_wo_memory_results_on_train_set.json",
        help='Path to DAMO results JSON file'
    )
    parser.add_argument(
        '--database-mapping',
        type=str,
        default=None,
        help='Path to database mapping file (optional)'
    )
...
def main():
    ...
    if not results_path.exists():
        logger.error(f"DAMO results file not found: {results_path}")
        sys.exit(1)
    
    # Quick stats
    try:
        stats = DAMODataLoader.count_incorrect(results_path)
        logger.info(f"\nDAMO Results Summary:")
        logger.info(f"  Total results: {stats['total']}")
        logger.info(f"  Incorrect: {stats['incorrect']}")
        logger.info(f"  Correct: {stats['correct']}")
        logger.info(f"  Error rate: {stats['incorrect']/stats['total']*100:.1f}%\n")
    except Exception as e:
        logger.warning(f"Could not read results summary: {e}")
    
    # Run pipeline
    pipeline = DAMOErrorAnalysisPipeline(args)
    pipeline.run()
